// from https://github.com/sfreiberg/simplessh
package main
...
func main() {
	...
	pass := os.Args[1]
	testConnectWithPassword(host, user, pass)
}

func testConnectWithPassword(host string, user string, pass string) {

	client, err := ConnectWithPassword(host + ":22", user, pass)
	if err != nil {
		panic(err)
	}
	defer client.Close()

	client.Upload("/tmp/hello.txt", "/tmp/hello.txt")

	fmt.Println("done")
}

func testConnectWithKeyFile(host string, user string) {
	/*
		Leave privKeyPath empty to use $HOME/.ssh/id_rsa.
		If username is blank simplessh will attempt to use the current user.
	*/
	client, err := ConnectWithKeyFile(host + ":22", user, "/Users/Wang/.ssh/id_rsa")
	if err != nil {
		panic(err)
	}
	defer client.Close()
